...
class TransactionHelper
{
    ...
    public const CRYPTO_DETAILS = 'crypto_details_info';
    ...
    public function createTransaction(OrderInterface $order, array $paymentData): ?string
    {
        try {
            ...
            $payment->setAdditionalInformation(
                [self::CRYPTO_DETAILS=> (array) $paymentData]
            );
            $formatedPrice = $order->getGrandTotal();

            $message = __(
                'Transaction is in processing. The authorized amount is %1 %2. Waiting for capture.',
                $formatedPrice,
